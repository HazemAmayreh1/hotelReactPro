          ...
          {room_info.map(room => (
            <div key={room.id} className="flex flex-col gap-y-4 h-fit p-4 border border-gray-200 rounded-lg shadow-lg">
              ...
              <ul className="flex flex-col list-disc list-inside gap-2 text-lg text-gray-600">
                ...
                <li>{room.peopl} people</li>
              </ul>
              <div className="flex justify-between items-center mt-4">
                <h3 className="text-xl text-purple-600 font-semibold">{room.price_per_night} per night</h3>
                <button className="bg-purple-600 text-white px-4 py-2 rounded-lg" onClick={handelchange}>Book Now</button>
              </div>
            </div>
          ))}
        </div>
